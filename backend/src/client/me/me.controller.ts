...
import {
  GetMeRewardsQueryDto,
  MeRewardsResponseDto,
} from './dto/me-rewards.dto';
...
export class MeController {
  ...
  async rewards(@Req() req: any, @Query() q: GetMeRewardsQueryDto) {
    ...
    return this.me.getRewardsSummary({
      companyId,
      days: q.days,
      musicId: q.musicId,
    });
  ...
  async plays(
    @Req() req: any,
    @Query() q: GetMePlaysQueryDto,
  ): Promise<MePlaysResponseDto> {
    const companyId = Number(req.user?.sub);
    return this.me.getPlays({
      companyId,
      musicId: q.musicId,
      page: q.page,
      limit: q.limit,
  ...
  async convertRewards(
    @Req() req,
    @Body() body: { amount: number; destination?: string; as?: 'xrp' | 'iou' },
  ) {
    const companyId = Number(req.user.sub);
    return this.me.convertRewards(companyId, body);
  }
}
