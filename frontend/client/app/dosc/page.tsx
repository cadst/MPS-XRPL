    ...
    <main className="mx-auto w-full max-w-5xl px-4 py-8 sm:py-10 md:px-8">
      ...
      <header className="mb-6 sm:mb-10 space-y-2">
        ...
        <p className="text-[13px] sm:text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">
          MPS에서 제공되는 API를 통해 외부 서비스에서 음원을 이용하는데 필요한
          설명을 제공하는 가이드입니다. <br className="hidden sm:block" />
          인증은 API Key 기반이며, 스트리밍은 HTTP Range + 진행 토큰(
          <code>X-Play-Token</code>)을 사용합니다.
        </p>
      </header>
      ...
      <section
        id="overview"
        className="scroll-mt-24 sm:scroll-mt-28 mb-12 sm:mb-16"
      >
        <h2 className="mb-3 sm:mb-5 text-lg sm:text-xl font-semibold leading-tight text-zinc-900 dark:text-white">
          1. 개요
        </h2>
        <div className="rounded-xl border border-zinc-200/80 bg-white/60 p-4 sm:p-6 text-[13px] sm:text-sm leading-relaxed shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-zinc-900/40">
          <ul className="list-disc space-y-2 pl-5 text-zinc-600 dark:text-zinc-400">
            ...
            <li>
              모든 요청은 <strong>HTTPS</strong> 로 전송해야 합니다.
            </li>
            <li>
              <span className="font-medium">순차 Range</span> 요청만 수행하면
              서버가 진행/검증을 자동 관리합니다.
            </li>
            <li>
              <code>X-Play-Token</code> 은 세션 식별 토큰이며 항상 최신 값을
              연속 요청에 포함합니다.
            </li>
          </ul>
        </div>
      ...
      >
        ...
        <div className="rounded-xl border border-zinc-200/80 bg-white/60 p-4 sm:p-6 text-[13px] sm:text-sm leading-relaxed shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-zinc-900/40">
          <p className="mb-3 sm:mb-4 font-medium text-zinc-800 dark:text-zinc-200">
            API Key
          </p>
          <ul className="list-disc space-y-1 pl-5 text-zinc-600 dark:text-zinc-400">
            <li>
              발급받은 키를 <code>X-API-Key</code> 헤더에 포함합니다.
            </li>
            <li>
              대체(권장 X): <code>?api_key=YOUR_KEY</code> 쿼리 파라미터 가능
              (서버는 헤더 우선).
            </li>
            <li>
              노출 방지를 위해 클라이언트(웹) 직접 삽입 대신 서버 간 프록시
              권장.
            </li>
          </ul>
          <div className="mt-3 sm:mt-4 rounded-md bg-zinc-950 p-3 sm:p-4 font-mono text-[12px] sm:text-xs text-zinc-200 dark:bg-zinc-900 overflow-auto">
            <pre>
              <code>{`# 예시\ncurl -H "X-API-Key: YOUR_API_KEY" https://api.example.com/music/123/play?pt=...`}</code>
            </pre>
          </div>
        </div>
      </section>
      ...
      >
        ...
        <div className="space-y-4 sm:space-y-5 rounded-xl border border-zinc-200/80 bg-white/60 p-4 sm:p-6 text-[13px] sm:text-sm leading-relaxed shadow-sm backdrop-blur-sm text-zinc-600 dark:border-white/10 dark:bg-zinc-900/40 dark:text-zinc-400">
          <p>
            엔드포인트: <code>GET /music/:music_id/play</code>
          </p>
          ...
          <ul className="list-disc pl-5 space-y-1">
            <li>
              <code>X-API-Key</code>: 기업 인증
            </li>
            <li>
              <code>Range</code> 헤더: 부분 전송 (예: <code>bytes=0-</code>).
              초기 요청에 Range 누락 시 서버가 기본 청크를 보냅니다.
            </li>
            <li>
              <code>X-Play-Token</code>: 진행 토큰 (첫 응답 헤더에서 수신 → 이후
              요청에 그대로 전달)
            </li>
            <li>
              <code>pt</code> 쿼리 또는 <code>pt</code> 쿠키: 토큰 대체 전송
              수단 (모바일/브라우저 호환)
            </li>
          </ul>
          <p className="font-medium text-zinc-800 dark:text-zinc-200">
          ...
          <ul className="list-disc pl-5 space-y-1">
            <li>
              <code>206 Partial Content</code> (Range 사용 시)
            </li>
            <li>
              <code>Content-Range</code>, <code>Accept-Ranges: bytes</code>
            </li>
            <li>
              <code>Content-Type: audio/mpeg</code> (파일 형식에 따라 다를 수
              있음)
            </li>
            <li>
              <code>X-Play-Token</code> & <code>Set-Cookie: pt=...</code>
            </li>
            <li>
              캐시 방지 헤더(<code>Cache-Control: no-store</code>)
            </li>
          </ul>
          <p className="font-medium text-zinc-800 dark:text-zinc-200">
            재생 흐름 요약
          ...
          <ol className="list-decimal pl-5 space-y-1">
            <li>
              API Key 포함 최초 요청 (권장: <code>Range: bytes=0-</code>)
            </li>
            <li>
              응답에서 <code>X-Play-Token</code> 추출
            </li>
            <li>다음 Range 청크 요청마다 동일 토큰 전송</li>
            <li>파일 끝까지(또는 필요한 구간까지) 순차 진행</li>
          </ol>
          <p className="text-[12px] sm:text-xs text-zinc-500 dark:text-zinc-500">
            유효재생 로직은 서버 내부에서 자동 처리되며, 클라이언트는 순차 Range
            요청만 수행하면 됩니다.
          </p>
        </div>
      </section>
      ...
      >
        ...
        <div className="rounded-xl border border-zinc-200/80 bg-white/60 p-4 sm:p-6 text-[13px] sm:text-sm leading-relaxed shadow-sm backdrop-blur-sm text-zinc-600 dark:border-white/10 dark:bg-zinc-900/40 dark:text-zinc-400 space-y-3 sm:space-y-4">
          <p>
            토큰은 재생 세션 상태(어떤 음원/기업인지, 서버가 추적한 전송 진행도
            등)를 안전하게 식별하는 값입니다.
          </p>
          <ul className="list-disc pl-5 space-y-1">
            <li>음원 ID 또는 회사가 바뀌면 새 토큰이 발급될 수 있음.</li>
            <li>
              토큰이 오래되었거나 손상되면 서버가 새 토큰을 발급 (헤더/쿠키로
              교체).
            </li>
            <li>
              항상 최신 응답 헤더의 <code>X-Play-Token</code> 값을 다음 요청에
              사용하십시오.
            </li>
      ...
      >
        ...
        <div className="space-y-3 sm:space-y-4 rounded-xl border border-zinc-200/80 bg-white/60 p-4 sm:p-6 text-[13px] sm:text-sm leading-relaxed shadow-sm backdrop-blur-sm text-zinc-600 dark:border-white/10 dark:bg-zinc-900/40 dark:text-zinc-400">
          <p>
            엔드포인트: <code>GET /lyric/:music_id/download</code>
          </p>
          <ul className="list-disc pl-5 space-y-1">
            <li>
              <code>X-API-Key</code> 필수
            </li>
            <li>
              응답: <code>200 OK</code> +{" "}
      ...
      <section
        id="examples"
        className="scroll-mt-24 sm:scroll-mt-28 mb-16 sm:mb-20"
      >
        <h2 className="mb-3 sm:mb-5 text-lg sm:text-xl font-semibold leading-tight text-zinc-900 dark:text-white">
          7. 예시 코드
        </h2>
