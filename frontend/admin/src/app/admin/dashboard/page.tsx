...
export default function DashboardPage() {
  ...
  const [topTracks, setTopTracks] = useState<
    Array<{ rank: number; validPlays: number; totalPlays: number }>
  >(
  ...
  const [realtimeApiStatus, setRealtimeApiStatus] = useState<
    Array<{
      id: number;
      status: string;
      endpoint: string;
  ...
  const [apiStatusFilter, setApiStatusFilter] = useState<
    "all" | "music" | "lyrics"
  >("all");
  const [realtimeTopTracks, setRealtimeTopTracks] = useState<
    Array<{
      id: number;
      rank: number;
      title: string;
  ...
  useEffect(() => {
    ...
    const wsUrl = baseUrl.replace(/^http/, "ws"); // baseUrl.replace(/^https?:\/\//, 'wss://').replace(/^http:\/\//, 'ws://')
    ...
    newSocket.on("realtime-update", (data) => {
      ...
      const parsed = (data.apiCalls || []).map((item: any) => {
        ...
        const midFromPayload =
          (item.musicId ?? item.music_id) !== undefined
            ? Number(item.musicId ?? item.music_id)
            : undefined;
        ...
        return {
          ...
          callType:
            item.callType ||
            (endpoint.includes("lyrics") ? "가사 호출" : "음원 호출"),
  ...
  useEffect(() => {
    ...
    const fetchHourly = async () => {
      try {
        ...
        const res = await fetch(
          `${baseUrl}/admin/companies/stats/hourly-plays`
        );
    ...
    const fetchRealtimeData = async () => {
      ...
      if (!isConnected) {
        try {
          console.log(
            "⚠️ WebSocket 연결되지 않음, HTTP API로 실시간 데이터 조회..."
          );
          const baseUrl =
            process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:3000";
    ...
    const fetchMissingTitles = async () => {
      try {
        ...
        const results = await Promise.allSettled(
          missing.map((id) =>
            fetch(`${baseUrl}/admin/musics/${id}`).then((r) =>
              r.ok ? r.json() : null
            )
          )
        );
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="text-center py-2 px-3 text-xs font-medium text-white/60">
                      ID
                    </th>
                    <th className="text-center py-2 px-3 text-xs font-medium text-white/60">
                      유효재생
                ...
                <tbody className="text-sm">
                  {(() => {
                    console.log(
                      "🔍 렌더링 시 realtimeApiStatus:",
                      realtimeApiStatus
                    );
                    console.log(
                      "🔍 렌더링 시 realtimeApiStatus.length:",
                      realtimeApiStatus.length
                    );
                    return realtimeApiStatus.length > 0 ? (
                      realtimeApiStatus
                        .filter((item) => {
                          if (apiStatusFilter === "all") return true;
                          if (apiStatusFilter === "music")
                            return item.callType === "음원 호출";
                          if (apiStatusFilter === "lyrics")
                            return item.callType === "가사 호출";
                          return true;
                        })
                        .map((item, idx) => (
                          <tr key={idx} className="border-b border-white/5">
                            <td className="py-2 px-3 text-white/60 font-mono text-xs text-center">
                              {item.id}
                            </td>
                            ...
                            <td className="py-2 px-3 text-white/80 text-center max-w-[260px]">
                              <span
                                title={
                                  item.musicTitle ||
                                  (item.musicId
                                    ? musicTitleById[item.musicId] ||
                                      `#${item.musicId}`
                                    : "")
                                }
                                className="inline-block truncate align-top max-w-[240px]"
                              >
                                {item.musicTitle ||
                                  (item.musicId
                                    ? musicTitleById[item.musicId] ||
                            ...
                            <td className="py-2 px-3 text-white/60 text-center">
                              {item.company}
                            </td>
                            <td className="py-2 px-3 text-white/40 text-center font-mono text-xs">
                      ...
                      <tr>
                        <td
                          colSpan={6}
                          className="py-4 px-3 text-center text-white/40"
                        >
                          데이터를 불러오는 중...
                        </td>
                      </tr>
